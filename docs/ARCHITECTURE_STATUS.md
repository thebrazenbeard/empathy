...
|---|---|---|
| `README.md` | CURRENT REPOSITORY GUIDE | Landing/status surface for the empathy repository. |
...
| `docs/candidates/PREACTION_FIDELITY_APPRAISAL_ENVELOPE_V1.md` | ARCHITECTURE CANDIDATE / HISTORICAL YIN-005 DISTILLATION | Minimal pre-action fidelity/appraisal mechanism plus Yang-005 falsification surface. Preserved for review/qualification; not promoted to runtime authority. |
...
5. privacy scope;
6. whether status is design, implementation, installation, activation, or behavioral qualification.

Do not collapse those states into a generic `current` label.

## Chat independence

The empathy project must not require a persistent worker chat.

Worker execution may occur in an ephemeral ChatGPT context, Work task, API/CLI invocation, subagent, or other runtime. Durable role, source, current routing, checkpoint, and evidence must live in GitHub/Bus state. Old ChatGPT URLs or issue-thread coordination may remain historical provenance but are not recovery prerequisites.

A broader Yin/Yang worker-reconstruction candidate is being reviewed separately in empathy Draft PR #3. This branch intentionally does not duplicate that worker contract; it preserves the distinct Yin-005 mechanism that PR #3 did not contain at the 2026-09-19 reconciliation cut. Neither Draft PR is runtime authority or a merge decision.
